import { FetchStockPricesUsecase } from './fetch-stock-prices.usecase';
import { YahooFinanceClient } from '../clients/yahoo-finance.client';
import { StockRepository } from '../repositories/stock.repository';
import { PriceRepository } from '../repositories/price.repository';
import { CliArgsService, CliArgs } from '../services/cli-args.service';
import { LoggerService } from '../services/logger.service';
import { Stock } from 'stock-analyze-domain';
import { Price } from 'stock-analyze-domain';
import { vi, describe, it, expect, beforeEach, afterEach } from 'vitest';

describe('FetchStockPricesUsecase', () => {
  let usecase: FetchStockPricesUsecase;
  let mockYahooFinanceClient: vi.Mocked<YahooFinanceClient>;
  let mockStockRepository: vi.Mocked<StockRepository>;
  let mockPriceRepository: vi.Mocked<PriceRepository>;
  let mockCliArgsService: vi.Mocked<CliArgsService>;
  ...
  beforeEach(() => {
    ...
    usecase = new FetchStockPricesUsecase(
      mockYahooFinanceClient,
      mockStockRepository,
      mockPriceRepository,
      mockCliArgsService,
  ...
  it('should process all stocks by default', async () => {
    await usecase.execute(['node', 'script.ts']);
  ...
  it('should process specified stocks when --codes is provided', async () => {
    ...
    await usecase.execute(['node', 'script.ts', '--codes', '1234,9012']);
  ...
  it('should fetch data for a specified period in manual mode', async () => {
    ...
    await usecase.execute(['node', 'script.ts', '--codes', '1234', '--start-date', '2023-01-01', '--end-date', '2023-01-31']);
  ...
  it('should perform initial data fetch in auto mode if directory does not exist', async () => {
    ...
    await usecase.execute(['node', 'script.ts', '--codes', '1234']);
  ...
  it('should handle no historical data for specified period', async () => {
    ...
    await usecase.execute(['node', 'script.ts', '--codes', '1234', '--start-date', '2024-01-01']);
